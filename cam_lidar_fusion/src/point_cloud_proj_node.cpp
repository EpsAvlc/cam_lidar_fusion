...
void UpdateParam(ros::NodeHandle nh_local_)
{
    ...
    fs["velo2cam_R"] >> velo2cam_R;
    fs["velo2cam_t"] >> velo2cam_t;
    fs["cam_proj"] >> cam_proj;
...
void ProjectPointCloudOnImagePlaner(const pcl::PointCloud<pcl::PointXYZ>& transformed_cloud, vector<Point3d>& fit_points)
{
    ...
    Eigen::MatrixXd camera_proj_eigen(3,4);
    // ROS_INFO("BEFOR EIGENLIZED");
    cv2eigen(cam_proj, camera_proj_eigen);
    // ROS_INFO("AFTER EIGENLIZED");
    Eigen::MatrixXd points(4, transformed_cloud.size());
    ...
    Eigen::MatrixXd points_pixel = camera_proj_eigen * rect_R_eigen * points;
    for(int i = 0; i < transformed_cloud.size(); ++i)
    {
        Point3d tmp;
...
void Callback(const sensor_msgs::ImageConstPtr& image, const sensor_msgs::PointCloud2ConstPtr& cloud)
{
    ...
    ROS_INFO("Projected_point size: %d", (int)pcl_cloud.size());
    Eigen::Matrix3d velo2cam_R_eigen;
    cv2eigen(velo2cam_R, velo2cam_R_eigen);
    Eigen::Vector3d velo2cam_t_eigen;
    cv2eigen(velo2cam_t, velo2cam_t_eigen);
    Eigen::Isometry3d transform;
    transform.setIdentity(); 
    transform.rotate(velo2cam_R_eigen);
    transform.translate(velo2cam_t_eigen);

    pcl::PointCloud<pcl::PointXYZ> transformed_cloud;
    pcl::transformPointCloud(pcl_cloud, transformed_cloud, transform.matrix());
    ...
    ROS_INFO("Projected_point size: %d", (int)proj_points.size());
    ROS_INFO("Img channels: %d", src_img.channels());
    for(int i = 0; i < proj_points.size(); ++i)
    {
        Scalar color(saturate_cast<uchar>(proj_points[i].z / 50. * 255));
